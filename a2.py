...
class CommandInterface:
    ...
    def is_valid_line(self, line):
        ...
        consecutive = 1
        half_length = len(line) / 2  # Use float division to determine half
        ...
        for i in range(len(line)):
            if line[i] is not None:
                if i > 0 and line[i] == line[i - 1]:
                    consecutive += 1
                    if consecutive >= 3:
                        return False  # No three consecutive same numbers
                else:
                    consecutive = 1
        ...
        # Use math.ceil to round up and check counts against half of line length
        if count_zero > math.ceil(half_length) or count_one > math.ceil(half_length):
            return False
    ...
    def solve(self, args):
        ...
        time_limit = self.time_limit - 0.1
        self.transposition_table = TranspositionTable()
        
        try:
            can_win, best_move = self.minimax(self.player, start_time, time_limit)
            
            if can_win and best_move:
                x, y, num = best_move
                print(f"{self.player} {x} {y} {num}") # current player wins by force
            elif can_win:
                print(f"{self.player}") # current player wins
            else:
                print(f"{3 - self.player}") # current player loses
    ...
    def minimax(self, player, start_time, time_limit):
        ...
        if lookup is not None:
            return lookup, None  # no move when using lookup
    
        #get all legal moves for current player
        moves = self.get_legal_moves()
        if not moves:
            self.transposition_table.store(state, False)
            return False, None  # Current player loses
    
        best_move = None  # Store the best winning move, init to None
        ...
        for move in moves:
            ...
            # Recursively call minimax to check if opponent can win 
            can_opponent_win, _ = self.minimax(3 - player, start_time, time_limit)
            win = not can_opponent_win
            ...
            if win:
                ...
                best_move = move  # Store the first winning move
                return True, best_move  # Return True and the winning move
